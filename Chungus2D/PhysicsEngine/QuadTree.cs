...
namespace Chungus2D.PhysicsEngine
{
    ...
    public class QuadTree<T>
    {
        ...
        public QuadTree(int splitCount, int depthLimit, ref Prism region)
        {
		...
		/// <param name="depthLimit">Maximum distance a node can be from the tree root.</param>
        /// <param name="region">The region that your quadtree occupies, all inserted quads should fit into this.</param>
		public QuadTree(int splitCount, int depthLimit, Prism region)
        ...
        public QuadTree(int splitCount, int depthLimit, float x, float y, float width, float height)
            : this(splitCount, depthLimit, new Prism(new Vector3(x, y,0),x + width, y + height, 0))
        {
        ...
        public void Insert(T value, ref Prism quad)
        {
            Leaf leaf;
        ...
        public void Insert(T value, Prism quad)
        {
        ...
        {
            var quad = new Prism(new Vector3(x,y,0), x + width, y + height,0);
            Insert(value, ref quad);
        }
        ...
        public bool SearchArea(ref Prism quad, ref List<T> values)
        {
            if (values != null)
                values.Clear();
            else
        ...
        /// <param name="quad">The area to search.</param>
        /// <param name="values">A list to populate with the results. If null, this function will create the list for you.</param>
        public bool SearchArea(Prism quad, ref List<T> values)
        {
            ...
            if (leafLookup.TryGetValue(value, out leaf))
            {
                ...
                if (branch.Leaves.Count > 0)
                    for (int i = 0; i < branch.Leaves.Count; ++i)
                        if (leaf != branch.Leaves[i] && leaf.Quad.IntersectsNoZ(ref branch.Leaves[i].Quad))
                ...
                while (branch != null)
                {
                    if (branch.Leaves.Count > 0)
                        for (int i = 0; i < branch.Leaves.Count; ++i)
                            if (leaf.Quad.IntersectsNoZ(ref branch.Leaves[i].Quad))
                                values.Add(branch.Leaves[i].Value);
                    branch = branch.Parent;
                }
            }
        ...
        static Branch CreateBranch(QuadTree<T> tree, Branch parent, int branchDepth, ref Prism quad)
        {
            ...
            float midY = quad.Y + (quad.Length - quad.Y) * 0.5f;
            branch.Quads[0].Set(quad.X, quad.Y,0, midX, midY);
            branch.Quads[1].Set(midX, quad.Y,0, quad.Width, midY);
            branch.Quads[2].Set(midX, midY,0, quad.Width, quad.Length);
            branch.Quads[3].Set(quad.X, midY,0, midX, quad.Length);
            return branch;
        }

        static Leaf CreateLeaf(T value, ref Prism quad)
        {
        ...
        internal class Branch
        {
            ...
            internal Prism[] Quads = new Prism[4];
            ...
            internal void Insert(Leaf leaf)
            {
                ...
                if (Split)
                {
                    for (int i = 0; i < 4; ++i)
                    {
                        if (Quads[i].ContainsNoZ(ref leaf.Quad))
                        {
                            if (Branches[i] == null)
            ...
            internal void SearchQuad(ref Prism quad, List<T> values)
            {
                if (Leaves.Count > 0)
                    for (int i = 0; i < Leaves.Count; ++i)
                        if (quad.IntersectsNoZ(ref Leaves[i].Quad))
